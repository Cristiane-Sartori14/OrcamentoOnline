...
function gerarLinkAssinatura() {
  ...
  // Codifica para base64 e URI para passar na URL
  const jsonStr = JSON.stringify(dados);
  const encoded = btoa(encodeURIComponent(jsonStr));

  const urlAssinatura = `${window.location.origin}/assinatura.html?data=${encoded}`;

  // Copia o link para clipboard e mostra
  navigator.clipboard.writeText(urlAssinatura).then(() => {
    alert("Link para assinatura copiado para a área de transferência:\n" + urlAssinatura);
  }).catch(() => {
    alert("Link para assinatura:\n" + urlAssinatura);
  });
}
